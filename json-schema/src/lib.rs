...
fn generate_layout<F>(
	...
	match layout.description() {
		...
		Description::Enum(enm) => generate_enum_type(json, model, embedding, type_property, enm),
		...
		Description::Set(item_layout, _) => {
			generate_set_type(json, model, embedding, type_property, *item_layout)
		}
		Description::List(item_layout, _) => {
			generate_list_type(json, model, embedding, type_property, *item_layout)
		}
	}
}
...
fn generate_struct<F>(
	...
	for field in s.fields() {
		...
		if let Some(description) = field.preferred_label(model) {
			layout_schema.insert(
				"description".into(),
				remove_newlines(description.trim()).into(),
			);
		}

		properties.insert(field.name().to_camel_case(), layout_schema.into());
...
fn embed_layout<F>(
	...
	match embedding.get(layout_ref) {
		Embedding::Reference => {
			generate_layout_ref(json, model, embedding, type_property, layout_ref)
		}
		Embedding::Indirect => generate_layout_defs_ref(json, model, layout_ref),
		Embedding::Direct => generate_layout(json, model, embedding, type_property, layout_ref),
	}
}
...
fn generate_layout_ref<F>(
	...
	match layout.description() {
		...
		Description::Enum(enm) => {
			generate_enum_type(json, model, embedding, type_property, enm)?;
			Ok(())
		}
		Description::Literal(lit) => {
			generate_literal_type(json, lit);
...
fn generate_set_type<F>(
	...
	def.insert("items".into(), item_schema.into());
