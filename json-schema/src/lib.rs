use iref::{IriRef, IriBuf};
use std::fmt;
use treeldr::{Ref, layout};
...
pub fn generate(model: &treeldr::Model, iri_ref: IriRef) -> Result<(), Error> {
	...
	let name = iri.path().file_name().ok_or_else(|| Error::InvalidLayoutIri(iri.clone()))?;
	let id = model.vocabulary().id(&iri).ok_or_else(|| Error::UndefinedLayout(iri.clone()))?;
	let layout_ref = model.require_layout(id, None).map_err(|e|
		match e.inner() {
	...
	json_schema.insert("$schema".into(), "https://json-schema.org/draft/2020-12/schema".into());
	...
	println!("{}", serde_json::to_string_pretty(&json_schema).map_err(Error::Serialization)?);

	Ok(())
}

fn generate_layout(json: &mut serde_json::Map<String, serde_json::Value>, model: &treeldr::Model, layout_ref: Ref<layout::Definition>) -> Result<(), Error> {
	...
	match layout.description().expect("unimplemented layout").inner() {
		Description::Struct(fields) => {
			generate_struct(json, model, fields)
		},
...
fn generate_struct(json: &mut serde_json::Map<String, serde_json::Value>, model: &treeldr::Model, fields: &treeldr::layout::Fields) -> Result<(), Error> {
...
fn generate_layout_ref(json: &mut serde_json::Map<String, serde_json::Value>, model: &treeldr::Model, layout_ref: Ref<layout::Definition>) -> Result<(), Error> {
	let layout = model.layouts().get(layout_ref).unwrap();
	// if let Some(description) = layout.preferred_documentation(model).short_description() {
	// 	json.insert("description".into(), description.trim().into());
	// }
...
fn generate_native_type(def: &mut serde_json::Map<String, serde_json::Value>, n: treeldr::layout::Native) {
	...
	match n {
		...
		Native::Time => {
			def.insert("type".into(), "string".into());
			def.insert("pattern".into(), "\\d\\d:\\d\\d:\\d\\d(\\.\\d+)?(([+-]\\d\\d:\\d\\d)|Z)?$".into());
		},
		Native::Date => {
			def.insert("type".into(), "string".into());
		...
		Native::DateTime => {
			def.insert("type".into(), "string".into());
			def.insert("pattern".into(), "^\\d{4}-\\d\\d-\\d\\dT\\d\\d:\\d\\d:\\d\\d(\\.\\d+)?(([+-]\\d\\d:\\d\\d)|Z)?$".into());
		}
		Native::Iri => {
			def.insert("type".into(), "string".into());
		}
