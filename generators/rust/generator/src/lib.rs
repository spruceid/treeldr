...
use quote::quote;
use rdf_types::{IriVocabulary, ReverseIriInterpretation, BlankIdBuf, Id};
use treeldr_layouts::{Ref, layout::{LayoutType, DataLayout, LiteralLayout, ListLayout}, Layouts, Layout, distill::RdfContext, Pattern};
...
impl<R> Options<R> {
	...
	where
		...
		R: Eq + Hash
	{
		default_layout_ident(rdf, layout_ref).or_else(|| {
			self.idents.get(layout_ref).cloned()
	...
	I: ReverseIriInterpretation<Iri = V::Iri>
{
	match pattern {
		...
		Pattern::Resource(r) => {
			rdf.interpretation.iris_of(r).next().map(|i| {
				Id::Iri(rdf.vocabulary.iri(i).unwrap().to_owned())
...
pub fn generate_intro_attribute(count: u32, offset: u32) -> TokenStream {
	let names = (offset..(count+offset)).map(|i| {
		i.to_string()
	});
...
where
	...
	I: ReverseIriInterpretation<Iri = V::Iri>,
{
	let input = input.iter().map(|p| {
		Ok(pattern_to_id(rdf, p)?.to_string())
	}).collect::<Result<Vec<_>, _>>()?;
	...
	I::Resource: Ord + Hash
{
	...
	match layout {
		...
		Layout::Product(layout) => {
			...
			let intro = generate_intro_attribute(layout.intro, layout.input);
			let dataset = generate_dataset_attribute(rdf, &layout.dataset)?;
			let fields = layout.fields.iter().map(|(name, f)| {
				let f_ident = syn::parse_str::<syn::Ident>(name.as_str()).map_err(|_| Error::InvalidFieldIdent(name.clone()))?;
		...
		Layout::Sum(layout) => {
			...
			let intro = generate_intro_attribute(layout.intro, layout.input);
			let dataset = generate_dataset_attribute(rdf, &layout.dataset)?;
			let variants = layout.variants.iter().map(|v| {
				let v_ident = syn::parse_str::<syn::Ident>(&v.name).map_err(|_| Error::InvalidVariantIdent(v.name.clone()))?;
		...
		Layout::List(layout) => {
			match layout {
				ListLayout::Unordered(layout) => {
					...
					let item_intro = generate_intro_attribute(layout.item.intro, layout.input + layout.intro);
				...
				ListLayout::Ordered(layout) => {
					...
					let node_intro = generate_intro_attribute(layout.node.intro, layout.input + layout.intro + 2);
				...
				ListLayout::Sized(layout) => {
					...
					let items = layout.items.iter().map(|item| {
						let item_intro = generate_intro_attribute(item.intro, layout.input + layout.intro);
