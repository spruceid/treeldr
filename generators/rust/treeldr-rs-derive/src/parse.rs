...
pub fn parse(input: syn::DeriveInput) -> Result<ParsedInput, Error> {
	let type_attrs = TypeAttributes::parse(input.attrs)?;
	let kind = type_attrs
		.kind
		.map(Ok)
		.unwrap_or_else(|| Kind::from_data(&input.data, input.ident.span()))?;
	...
	let layout = match kind {
		...
		Kind::Bytes => Layout::Literal(LiteralLayout::Data(DataLayout::ByteString(
			ByteStringLayout {
				type_: Default::default(),
				header: LayoutHeader {
					base: type_attrs.base,
				...
				match attr.meta {
					syn::Meta::List(meta) => {
						...
						while let Some(token) = tokens.next() {
							match token {
								TokenTree::Ident(ident) => match Kind::from_ident(&ident) {
									Some(k) => replace(&mut result.kind, k, ident.span())?,
									None => {
										if ident == "base" {
											let (value, span) = expect_compact_iri_argument(
												&mut tokens,
												ident.span(),
											)?;
										...
										if ident == "prefix" {
											let values = expect_binding_list_argument(
												&mut tokens,
												ident.span(),
											)?
											.0;
										...
										if ident == "head" {
											let (value, span) =
												expect_pattern_argument(&mut tokens, ident.span())?;
										...
										if ident == "input" {
											let (value, span) = expect_string_list_argument(
												&mut tokens,
												ident.span(),
											)?;
										...
										if ident == "dataset" {
											let (value, span) = expect_quad_list_argument(
												&mut tokens,
												ident.span(),
											)?;
			...
			if attr.path().is_ident("tldr") {
				match attr.meta {
					syn::Meta::List(meta) => {
						...
						while let Some(token) = tokens.next() {
							match token {
								TokenTree::Literal(lit) => match syn::Lit::new(lit) {
									syn::Lit::Str(s) => {
								...
								TokenTree::Ident(ident) => {
									if ident == "head" {
										let (value, span) =
											expect_string_argument(&mut tokens, ident.span())?;
										replace(&mut result.head, value, span)?;
									}
									...
									if ident == "intro" {
										let (value, span) =
											expect_string_list_argument(&mut tokens, ident.span())?;
										replace(&mut result.intro, value, span)?;
									}

									if ident == "dataset" {
										let (value, span) =
											expect_quad_list_argument(&mut tokens, ident.span())?;
										replace(&mut result.dataset, value, span)?;
									}

									if ident == "input" {
										let (value, span) = expect_pattern_list_argument(
											&mut tokens,
											ident.span(),
										)?;
										replace(&mut result.input, value, span)?;
									...
									if ident == "graph" {
										let (value, span) = expect_argument(
											&mut tokens,
											ident.span(),
											parse_graph_value,
										)?;
...
fn expect_argument<T>(
	...
	match tokens.next() {
		Some(TokenTree::Group(g)) => f(g.stream(), g.span()),
		Some(other) => Err(Error::UnexpectedToken(other)),
		None => Err(Error::MissingToken(span)),
	}
}

fn parse_string(tokens: TokenStream, span: Span) -> Result<(String, Span), Error> {
...
fn parse_compact_iri(tokens: TokenStream, span: Span) -> Result<(CompactIri, Span), Error> {
	let (s, span) = parse_string(tokens, span)?;
	Ok((
		CompactIri(IriRefBuf::new(s).map_err(|e| Error::InvalidCompactIri(e.0, span))?),
		span,
	))
}
...
fn parse_string_list(tokens: TokenStream, span: Span) -> Result<(Vec<String>, Span), Error> {
	parse_list(tokens, span, string_token)
}

fn parse_pattern_list(tokens: TokenStream, span: Span) -> Result<(Vec<Pattern>, Span), Error> {
	parse_list(tokens, span, pattern_token)
}

fn parse_quad_list(tokens: TokenStream, span: Span) -> Result<(Vec<Quad>, Span), Error> {
...
fn parse_graph_value(tokens: TokenStream, span: Span) -> Result<(GraphValue, Span), Error> {
	let mut tokens = tokens.into_iter();
	match tokens.next() {
	...
	while let Some(token) = tokens.next() {
		...
		match tokens.next() {
			Some(TokenTree::Punct(p)) if p.as_char() == '=' => match tokens.next() {
				Some(token) => {
					let value = CompactIri(
						IriRefBuf::new(string_token(token)?.0)
							.map_err(|e| Error::InvalidCompactIri(e.0, span))?,
					);
