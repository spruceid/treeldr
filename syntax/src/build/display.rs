...
pub trait Display {
	fn fmt(&self, namespace: &Namespace, f: &mut fmt::Formatter) -> fmt::Result;

	fn display<'n>(&self, namespace: &'n Namespace) -> Displayed<'n, '_, Self> {
		Displayed(namespace, self)
	}
}

pub struct Displayed<'n, 'a, T: ?Sized>(&'n Namespace, &'a T);
...
impl Display for super::Subject {
	fn fmt(&self, namespace: &Namespace, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
