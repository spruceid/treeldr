...
impl<F: Clone> LocalContext<F> {
		...
		F: Clone + Ord
	{
		...
		if bind_to_layout {
			context.get_mut(*id).unwrap().as_layout_mut().unwrap().set_type(*id, Some(loc.clone()))?;
		}
		...
		F: Clone + Ord
	{
		...
		context.get_mut(*id).unwrap().as_layout_mut().unwrap().set_literal(regexp, Some(loc.clone()))?;
		Ok(())
	}
		...
		match self.literal.entry(lit) {
			...
			Entry::Vacant(entry) => {
				...
				let id = self.next_id.take().unwrap_or_else(|| {
					Loc(Id::Blank(context.vocabulary_mut().new_blank_label()), loc.clone())
				});
...
impl<F: Clone + Ord> treeldr_build::Document<F, Descriptions> for crate::Document<F> {
	...
	fn declare(&self, local_context: &mut Self::LocalContext, context: &mut Context<F, Descriptions>) -> Result<(), Error<F>> {
	...
	fn relate(self, local_context: &mut Self::LocalContext, context: &mut Context<F, Descriptions>) -> Result<(), Error<F>> {
		for ty in self.types {
...
pub trait Declare<F: Clone + Ord> {
	fn declare(&self, local_context: &mut LocalContext<F>, context: &mut Context<F, Descriptions>) -> Result<(), Error<F>>;
}

pub trait Build<F: Clone + Ord> {
	type Target;

	fn build(self, local_context: &mut LocalContext<F>, context: &mut Context<F, Descriptions>) -> Result<Self::Target, Error<F>>;
}
	...
	fn build(
		...
		let iri = match id {
			crate::Id::Name(name) => {
				...
				iri_ref.resolve(local_context.base_iri(context.vocabulary_mut(), loc.clone())?.as_iri());
				iri_ref.try_into().unwrap()
			}
			crate::Id::IriRef(iri_ref) => iri_ref.resolved(local_context.base_iri(context.vocabulary_mut(), loc.clone())?.as_iri()),
			crate::Id::Compact(prefix, iri_ref) => {
				local_context.expand_compact_iri(&prefix, iri_ref.as_iri_ref(), &loc)?
			}
		};

		Ok(Loc(Id::Iri(Term::from_iri(iri, context.vocabulary_mut())), loc))
	}
}
	...
	fn build(
		...
		match def.description {
			...
			Loc(crate::TypeDescription::Alias(expr), expr_loc) => {
				local_context.next_id = Some(Loc(id, id_loc.clone()));
	...
	fn build(
		...
		match ty {
			...
			crate::OuterTypeExpr::Union(label, options) => {
				...
				let options_list = context.try_create_list_with::<Error<F>, _, _>(options, |ty_expr, context| {
			...
			crate::OuterTypeExpr::Intersection(label, types) => {
				...
				let types_list = context.try_create_list_with::<Error<F>, _, _>(types, |ty_expr, context| {
					let Loc(id, loc) = ty_expr.build(local_context, context)?;
					Ok(Caused::new(id.into_term(), Some(loc)))
				})?;
...
impl<F: Clone + Ord> Build<F> for Loc<crate::PropertyDefinition<F>, F> {
	...
	fn build(
		...
		let doc = def.doc.map(|doc| doc.build(local_context, context)).transpose()?;
		...
		let range = def.ty.map(|Loc(ty, _)| -> Result<_, Error<F>> {
		...
		context.declare_property(id, Some(loc.clone()));
		let node = context.get_mut(id).unwrap();
...
impl<F: Clone + Ord> Build<F> for Loc<crate::LayoutDefinition<F>, F> {
	...
	fn build(
		...
		context.get_mut(id).unwrap().as_layout_mut().unwrap().set_type(ty_id, Some(ty_id_loc))?;

		match def.description {
			Loc(crate::LayoutDescription::Normal(fields), fields_loc) => {
				let fields_list = context.try_create_list_with::<Error<F>, _, _>(fields, |field, context| {
				...
				context.get_mut(id).unwrap().as_layout_mut().unwrap().set_fields(fields_list, Some(fields_loc))?;
			}
...
impl<F: Clone + Ord> Build<F> for Loc<crate::Alias, F> {
	...
	fn build(
		...
		match treeldr::vocab::Name::new(name.as_str()) {
			...
			Err(_) => return Err(treeldr_build::Error::new(
	...
	fn build(
		...
		match ty {
			...
			crate::OuterLayoutExpr::Union(label, options) => {
				...
				let variants = context.try_create_list_with::<Error<F>, _, _>(options, |layout_expr, context| {
					...
					let (layout_expr, variant_name) = if layout_expr.value().expr.is_namable() {
						...
						(Loc(crate::NamedInnerLayoutExpr { expr, name: None }, loc), name)
					};

					let Loc(layout, layout_loc) = layout_expr.build(local_context, context)?;

					let variant_name = variant_name.map(|name| {
						name.build(local_context, context)
					}).transpose()?;
	
					context.declare_layout_variant(variant_id, Some(loc.clone()));

					let variant = context.get_mut(variant_id).unwrap().as_layout_variant_mut().unwrap();
			...
			crate::OuterLayoutExpr::Intersection(label, types) => {
				...
				let layouts_list = context.try_create_list_with::<Error<F>, _, _>(types, |layout_expr, context| {
...
impl<F: Clone + Ord> Build<F> for Loc<crate::NamedInnerLayoutExpr<F>, F> {
	...
	fn build(
		...
		if let Some(name) = this.name {
			...
			if is_namable {
				context.get_mut(id).unwrap().as_layout_mut().unwrap().set_name(name, Some(name_loc))?;
			} else {
...
impl<F: Clone + Ord> Build<F> for Loc<crate::FieldDefinition<F>, F> {
	...
	fn build(
		...
		let Loc(name, name_loc) = def.alias.unwrap_or_else(|| {
			match prop_id {
				Id::Iri(id) => {
					Loc(
						crate::Alias(id
							.iri(context.vocabulary())
							.unwrap()
							.path()
							.file_name()
...
impl<F: Clone + Ord> treeldr_build::layout::PseudoDescription<F> for LayoutDescription {
	...
	fn sub_layouts<D: treeldr_build::Descriptions<F>>(&self, context: &Context<F, D>, causes: &Causes<F>) -> Result<Vec<SubLayout<F>>, Self::Error> {
		match self {
	...
	fn reduce(self, _id: Id, _name: &MaybeSet<Name, F>, _ty: &MaybeSet<Id, F>) -> Result<treeldr_build::layout::Description, Self::Error> {
		match self {
