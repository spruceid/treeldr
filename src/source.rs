	...
	pub fn add(&mut self, path: Path, content: String) -> (Id, &File) {
		...
		let id = match self.map.entry(path) {
			...
			Entry::Vacant(entry) => {
				...
				self.files.push(File::new(entry.key().clone(), Buffer::new(content)));
				id
			}
		};
...
impl<'a> codespan_reporting::files::Files<'a> for Files {
	...
	fn name(&'a self, id: Id) -> Result<&'a Path, codespan_reporting::files::Error> {
		self.get(id).map(File::path).ok_or(codespan_reporting::files::Error::FileMissing)
	}

	fn source(&'a self, id: Id) -> Result<&'a Buffer, codespan_reporting::files::Error> {
		self.get(id).map(File::buffer).ok_or(codespan_reporting::files::Error::FileMissing)
	}

	fn line_index(&'a self, id: Id, byte_index: usize) -> Result<usize, codespan_reporting::files::Error> {
		self.get(id).map(|file| file.buffer().line_index(byte_index)).ok_or(codespan_reporting::files::Error::FileMissing)
	}

	fn line_range(&'a self, id: Id, line_index: usize) -> Result<Range<usize>, codespan_reporting::files::Error> {
		self.get(id).map(|file|
			file.buffer().line_range(line_index).ok_or_else(|| 
				codespan_reporting::files::Error::LineTooLarge { given: line_index, max: file.buffer().line_count() }
			)
		).transpose()?.ok_or(codespan_reporting::files::Error::FileMissing)
	}
}