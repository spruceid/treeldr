use std::{fmt, iter::Peekable, fmt::Debug};
use super::*;
use lexing::Token;
...
impl<E: Debug + fmt::Display> crate::Error for Loc<Error<E>> {
	...
	fn labels(&self) -> Vec<codespan_reporting::diagnostic::Label<source::Id>> {
		vec![
			codespan_reporting::diagnostic::Label::primary(self.source().file(), self.source().span()).with_message(self.to_string())
		]
	}
}
...
fn peek_token<E: Debug, L: Iterator<Item=Result<Loc<Token>, Loc<E>>>>(lexer: &mut Peekable<L>) -> Result<Option<Loc<Token>>, Loc<Error<E>>> {
	match lexer.peek() {
		None => return Ok(None),
		Some(Ok(token)) => return Ok(Some(token.clone())),
		Some(Err(_)) => Err(consume_token(lexer).unwrap_err())
	}
}

fn consume_token<E: Debug, L: Iterator<Item=Result<Loc<Token>, Loc<E>>>>(lexer: &mut Peekable<L>) -> Result<Option<Loc<Token>>, Loc<Error<E>>> {
	match lexer.next() {
		None => Ok(None),
...
fn consume_expected_token<E: Debug, L: Iterator<Item=Result<Loc<Token>, Loc<E>>>>(file: source::Id, lexer: &mut Peekable<L>, start: usize) -> Result<Loc<Token>, Loc<Error<E>>> {
	match lexer.next() {
		None => Err(Loc::new(Error::Unexpected(None), Source::new(file, start.into()))),
		Some(Ok(token)) => Ok(token),
		Some(Err(e)) => Err(e.map(Error::Lexer))
	}
}

fn parse_comma_separated_list<E: Debug, L: Iterator<Item=Result<Loc<Token>, Loc<E>>>, T: Parse>(file: source::Id, lexer: &mut Peekable<L>, start: usize) -> Result<Vec<Loc<T>>, Loc<Error<E>>> {
...
fn parse_block<E: Debug, L: Iterator<Item=Result<Loc<Token>, Loc<E>>>, T: Parse>(file: source::Id, lexer: &mut Peekable<L>, start: usize) -> Result<Loc<Vec<Loc<T>>>, Loc<Error<E>>> {
	let (token, span) = consume_expected_token(file, lexer, start)?.into_parts();

	match token {
		Token::Block(lexing::Delimiter::Brace, tokens) => {
			let mut block_lexer = tokens.into_iter().map(|token| Ok(token)).peekable();
...
impl Parse for Document {
	fn parse<E: Debug, L: Iterator<Item=Result<Loc<Token>, Loc<E>>>>(file: source::Id, lexer: &mut Peekable<L>, start: usize) -> Result<Loc<Self>, Loc<Error<E>>> {
		...
		while let Some(_) = peek_token(lexer)? {
	...
	fn parse<E: Debug, L: Iterator<Item=Result<Loc<Token>, Loc<E>>>>(file: source::Id, lexer: &mut Peekable<L>, start: usize) -> Result<Loc<Self>, Loc<Error<E>>> {
		...
		match token {
			Token::Keyword(lexing::Keyword::Type) => {
				...
				let (properties, prop_source) = parse_block(file, lexer, id.span().end())?.into_parts();
				span.set_end(prop_source.span().end());
				Ok(Loc::new(Item::Type(TypeDefinition {
					id,
					properties
				}), Source::new(file, span)))
			},
			Token::Keyword(lexing::Keyword::Layout) => {
				let id = Id::parse(file, lexer, span.end())?;
				let (fields, field_source) = parse_block(file, lexer, id.span().end())?.into_parts();
				span.set_end(field_source.span().end());
				Ok(Loc::new(Item::Layout(LayoutDefinition {
					id,
					fields
				}), Source::new(file, span)))
			}
			unexpected => Err(Loc::new(Error::Unexpected(Some(unexpected)), Source::new(file, span)))
		}
	}
}
...
fn infallible<T, E>(t: T) -> Result<T, E> {
	Ok(t)
}
		...
		let args = match peek_token(lexer)? {
			Some(token) => match token.into_parts() {
				(Token::Block(lexing::Delimiter::Parenthesis, tokens), args_source) => {
					consume_token(lexer)?;
					let mut block_lexer = tokens.into_iter().map(infallible).peekable();
					let items = parse_comma_separated_list::<E, _, _>(file, &mut block_lexer, start)?;
