...
pub trait FromRdf<T: Id>: Sized {
	fn from_rdf<G>(id: T::Ref<'_>, graph: &G) -> Result<Self, FromRdfError> where G: grdf::Graph<Subject=T, Predicate=T, Object=Object<T>>;
}
	...
	($literal:ident: $id:ident) => {
		match $literal {
			...
			Literal::LangString(_, _) => {
				return Err(FromRdfError::UnexpectedLangString)
			}
		}
	};
}
