				...
				match n.as_integer() {
					Some(i) => {
						if i.bits() <= 64 {
							...
							let tag = if i.is_positive() {
								2
							} else {
								3
							};

							serde_cbor::Value::Tag(tag, Box::new(
								serde_cbor::Value::Bytes(i.to_be_bytes())
							))
						}
					}
					None => {
						serde_cbor::Value::Float(n.to_f64())
					}
				}
			}
			Value::Literal(Literal::ByteString(bytes)) => {
				serde_cbor::Value::Bytes(bytes)
			}
			Value::Literal(Literal::TextString(string)) => {
				serde_cbor::Value::Text(string)
			}
			Value::Record(map) => {
				serde_cbor::Value::Map(map.into_iter().map(|(key, value)| {
					(
