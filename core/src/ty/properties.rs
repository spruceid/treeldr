use crate::{prop, prop::restriction::Contradiction, Causes, Ref};
...
impl<F> PropertyData<F> {
	pub fn add_causes(&mut self, causes: impl Into<Causes<F>>)
	where
		F: Ord,
	{
	...
	pub fn add_cause(&mut self, cause: Option<Location<F>>)
	where
		F: Ord,
	{
...
impl<F: Ord> Properties<F> {
	pub fn insert(
		...
		restrictions: Option<prop::Restrictions<F>>,
	...
	pub fn restrict(
		&mut self,
		prop: Ref<prop::Definition<F>>,
		restriction: prop::Restriction<F>,
			...
			(Some(mut excluded), Some(other_excluded)) => {
				for (prop, cause) in other_excluded {
					match excluded.entry(*prop) {
						Entry::Occupied(mut entry) => entry.get_mut().extend(cause.clone()),
						Entry::Vacant(entry) => {
...
impl<'a, F> IntoIterator for &'a Properties<F> {
